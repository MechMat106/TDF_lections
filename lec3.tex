...
\end{statement}
\begin{proof}
	Полином Жегалкина единственен, так как $c_{i_1}, \ldots, c_{i_n}$ однозначно определены $2^n$  коэффициентов $2^{2^n}$ -- набор значений коэффициентов, $2^{2^n}$ -- количество функций от $n$ переменных $\Rightarrow$ для каждой функции $\exists !$ полином Жегалкина. 
\end{proof}
